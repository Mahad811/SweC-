...
def create_instance(repo: Repo, pull: dict, output_path: str, mode: str ='swebench') -> dict:
    ...
    #     patch = ""
    #     test_patch = ""
    repo_full_name = repo.repo.get('full_name', '') if isinstance(repo.repo, dict) else getattr(repo.repo, 'full_name', '')
...
def is_valid_pull(pull: dict) -> bool:
    ...
    if pull.get("merged_at") is None:
        logger.info(f"PR {pull.get('number')} not merged")
        return False
    if "resolved_issues" not in pull or len(pull["resolved_issues"]) < 1:
        logger.info(f"PR {pull.get('number')} has no resolved_issues")
        return False
    return True
...
def is_valid_instance(instance: dict) -> bool:
    ...
    """
    if instance.get("patch") is None or instance["patch"] == "":
        logger.info(f"Instance {instance.get('pull_number')} no patch")
        return False
    if instance.get("problem_statement") is None or instance["problem_statement"] == "":
        logger.info(f"Instance {instance.get('pull_number')} no problem statement")
        return False
    return True
...
def main(pr_file: str, output: str, token: Optional[str] = None,mode: Optional[str] = 'swebench',language: Optional[str] = 'python', cutoff_date: Optional[str] = None):
    ...
    if cutoff_date is None:
        cutoff_date = "2025-03-31T23:59:59Z"
    cutoff_date_dt = datetime.strptime(cutoff_date, "%Y-%m-%dT%H:%M:%SZ")
    if token is None:
        ...
        with open(all_output) as f:
            for line in f:
                ...
                if "created_at" in pr and pr["created_at"] is not None:
                    ...
                    if pr_created_at >= cutoff_date_dt:
    ...
    with open(all_output, write_mode_all) as all_output_file:
        ...
        with open(output, write_mode) as output_file:
            for ix, line in enumerate(open(pr_file)):
                ...
                if ix % 100 == 0:
                    repo_full_name = pull['base']['repo'].get('full_name', '') if isinstance(pull['base']['repo'], dict) else ''
                    logger.info(
                ...
                repo_full_name = pull['base']['repo'].get('full_name', '') if isinstance(pull['base']['repo'], dict) else ''
                instance_id = (
                    repo_full_name + "-" + str(pull["number"])
                )
                ...
                safe_mode = mode if mode is not None else 'swebench'
                instance = create_instance(repo, pull, original_output_path, safe_mode)
