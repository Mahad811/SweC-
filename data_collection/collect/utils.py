        ...
        if line.startswith("diff --git a/"):
            ...
            if repo.language == 'python':
                ...
                if flag != "test" and not is_java:
                    flag = None
            elif repo.language == 'cpp':
                file_name = line.split("/")[-1]
                # Recognize C++ source/header files
                is_cpp_file = any(file_name.endswith(ext) for ext in [".cpp", ".cc", ".cxx", ".c", ".hpp", ".h"])
                # Recognize test files by name or path
                is_test = (
                    "test" in file_name.lower() or
                    file_name.lower().startswith("test") or
                    file_name.lower().endswith("_test.cpp") or
                    file_name.lower().endswith("_test.cc") or
                    file_name.lower().endswith("_test.cxx") or
                    file_name.lower().endswith("_test.hpp") or
                    file_name.lower().endswith("test.cpp") or
                    file_name.lower().endswith("test.cc") or
                    file_name.lower().endswith("test.cxx") or
                    file_name.lower().endswith("test.hpp") or
                    "/test/" in line.lower() or "/tests/" in line.lower()
                )
                if is_cpp_file:
                    if is_test:
                        flag = "test"
